...
interface JQuery {
    dataTable(param?: Options): DataTable;
    DataTable(param?: Options): DataTableAPI;
}

interface DataTableAPI extends DataTable {
    search(input: string, regex?: boolean, smart?: boolean, caseInsen?: boolean): DataTableAPI;
    search(): string;
    clear(): DataTableAPI;
    data(): DataTableAPI;
    destroy(remove?: boolean): DataTableAPI;
    draw(reset?: boolean): DataTableAPI;
    off(event: string, callback?: () => void): DataTableAPI;
    on(event: string, callback: () => void): DataTableAPI;
    one(event: string, callback: () => void): DataTableAPI;
    order(): any[];
    order(order: any[], ...additional: any[]): DataTableAPI;
    //order.listener()
    page: PageInterface;
    settings(): DataTableAPI;
...
interface PageInterface {
    (): number;
    (page: any): DataTableAPI;
    (page: string): DataTableAPI;
...
interface ColumnsInterface extends DataTableAPI {
    (modifer?: any): ColumnsInterface;
    index();
    cache();
    header(): Node;
    visible(): boolean;
    visible(show: boolean): DataTableAPI;
...
interface InitComplete {
    (settings: Settings, json: any): void;
}

interface PreDrawCallback {
...
interface StateLoadCallback {
    (settings: Settings): any;
}

interface StateLoadParams {
...
interface StateSaveCallback {
    (settings: any, data: any): void;
}

interface StateSaveParams {
    (settings: any, data: any): void;
}
